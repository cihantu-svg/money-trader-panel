...
def send_telegram_message(text):
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        log.warning("TELEGRAM_TOKEN/TELEGRAM_CHAT_ID eksik, Telegram'a gonderilemiyor.")
        return False
    try:
        r = session.post(
        ...
        if r.status_code != 200:
            log.error(f"Telegram mesaj hatasi: {r.status_code} {r.text[:200]}")
        return r.status_code == 200
    except Exception as e:
        log.error(f"Telegram mesaj gonderim hatasi: {e}")
        return False


def send_telegram_document(path, caption=""):
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID:
        log.warning("TELEGRAM_TOKEN/TELEGRAM_CHAT_ID eksik, dosya gonderilemiyor.")
        return False
    try:
        ...
        if r.status_code != 200:
            log.error(f"Telegram dosya hatasi: {r.status_code} {r.text[:200]}")
        return r.status_code == 200
    except Exception as e:
...
def split_telegram_message(text, max_len=3800):
    """Telegram tek mesaj limiti ~4096 karakter - guvenli pay birakarak boler."""
    chunks = []
...
def build_telegram_summary(signals_by_threshold, ls_cache):
    """Kisa/oz bir ozet mesaji hazirlar: her govde esigi icin baseline + en iyi
    onay ve en iyi fade satirini secer (tum detay CSV'de zaten var)."""
    lines = []
    lines.append("📊 <b>BIRLESIK BACKTEST SONUCU</b>")
    lines.append(f"{TOP_N_SYMBOLS} coin | {LOOKBACK_DAYS} gun | taker>=%{IMBALANCE_RATIO*100:.0f}")
    lines.append("=" * 30)

    best_overall = []  # (hit10, aciklama)
    ...
    for th in MIN_BODY_PCT_LIST:
        ...
        if n_all == 0:
            continue
        hits10 = sum(1 for s in sigs if s.hit_targets.get(10))
        rate10 = hits10 / n_all * 100
        lines.append(f"\n🕯️ Govde >=%{th:.1f}  (baseline n={n_all})")
        lines.append(f"   %10 hit-rate: {rate10:.1f}%")
        best_overall.append((rate10, f"Govde>=%{th:.1f} BASELINE n={n_all}"))

        with_ls = [s for s in sigs if s.long_account is not None]
        if not with_ls:
            continue

        # bu esik icin en iyi ONAY ve en iyi FADE kombinasyonunu bul (n>=10 sarti ile gurultu filtrele)
        best_confirm = None
        for lth in LS_THRESHOLDS:
            kept = [s for s in with_ls if (s.direction == "LONG" and s.long_account >= lth)
                    or (s.direction == "SHORT" and (1 - s.long_account) >= lth)]
            if len(kept) < 10:
                continue
            hits = sum(1 for s in kept if s.hit_targets.get(10))
            rate = hits / len(kept) * 100
            if best_confirm is None or rate > best_confirm[0]:
                best_confirm = (rate, lth, len(kept))

        if best_confirm:
            rate, lth, n = best_confirm
            lines.append(f"   ✅ En iyi ONAY: L/S>=%{lth*100:.0f} → n={n}, %10 hit-rate={rate:.1f}%")
            best_overall.append((rate, f"Govde>=%{th:.1f} ONAY L/S>=%{lth*100:.0f} n={n}"))

    lines.append("\n" + "=" * 30)
    best_overall.sort(key=lambda x: x[0], reverse=True)
    lines.append("🏆 <b>En iyi 3 kombinasyon (%10 hit-rate):</b>")
    for rate, desc in best_overall[:3]:
        lines.append(f"   {rate:.1f}% - {desc}")
...
def main():
    ...
    for th in MIN_BODY_PCT_LIST:
        ...
        # BASELINE (L/S ile eslesen tum sinyaller, filtresiz) - CSV'ye kaydet
        for s in with_ls:
            all_rows_for_csv.append({
                "body_threshold": th, "ls_mode": "baseline", "ls_threshold": None,
                "symbol": s.symbol, "direction": s.direction, "max_favorable_pct": round(s.max_favorable_pct, 2),
                "hit_10pct": s.hit_targets.get(10), "long_account": round(s.long_account, 4),
            })
        ...
        for lth in LS_THRESHOLDS:
            ...
            for s in with_ls:
                ...
                if s.direction == "LONG" and la >= lth:
                    ...
                    kept.append((s.symbol, "SHORT(faded)", max_fav, hit, la))
                elif s.direction == "SHORT" and (1 - la) >= lth:
                    max_fav, hit = fade_forward_move(df, s.entry_idx, "LONG")
                    kept.append((s.symbol, "LONG(faded)", max_fav, hit, la))
                else:
                    kept.append((s.symbol, s.direction, s.max_favorable_pct, s.hit_targets, la))
            n = len(kept)
            rates = {}
            for t in TARGET_PCTS:
                hits = sum(1 for _, _, _, ht, _ in kept if ht.get(t))
                rates[t] = (hits / n * 100) if n else 0
            avg_fav = (sum(f for _, _, f, _, _ in kept) / n) if n else 0
            row = f"    {lth*100:>8.0f}% | {n:>5} |"
            for t in TARGET_PCTS:
                row += f" {rates[t]:>4.1f}|"
            row += f" {avg_fav:>7.2f}%"
            print(row)
            for sym, direc, fav, ht, la in kept:
                all_rows_for_csv.append({
                    "body_threshold": th, "ls_mode": "fade", "ls_threshold": lth,
                    "symbol": sym, "direction": direc, "max_favorable_pct": round(fav, 2),
                    "hit_10pct": ht.get(10), "long_account": round(la, 4),
                })
    ...
    if TELEGRAM_TOKEN and TELEGRAM_CHAT_ID:
        summary_text = build_telegram_summary(signals_by_threshold, ls_cache)
        for chunk in split_telegram_message(summary_text):
            send_telegram_message(chunk)
            time.sleep(0.5)
        sent = send_telegram_document("combined_results.csv", caption="Detayli backtest sonuclari (tum sinyaller)")
        if sent:
            log.info("Ozet ve CSV Telegram'a gonderildi.")
        else:
            log.warning("CSV Telegram'a gonderilemedi (ozet mesaji gonderilmis olabilir).")
    else:
        log.info("TELEGRAM_TOKEN/TELEGRAM_CHAT_ID tanimli degil, sadece konsol+CSV ciktisi verildi.")
