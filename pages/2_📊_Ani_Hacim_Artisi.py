# -*- coding: utf-8 -*-
"""
MONEY TRADER - ANİ HACİM ARTIŞI TARAMASI (BINANCE FUTURES)
...
⚠️ fapi.binance.com ABD IP'lerinde 451 hatası verebilir — Frankfurt (Render) gerektirir.
"""
import time
...
def get_volume_spike(symbol, interval, server_time_ms, lookback=20, min_elapsed_pct=5):
    """
    Son kapanmamış mumun normalize hacmini hesaplar.

    Normalize hacim = (şu ana kadar gelen hacim / geçen süre oranı) / (son N mumun ortalama hacmi)
    Örn: 4 saatlik mumun %25'i geçmişken, o süreye oranlanmış hacim normalin 5 katıysa → 5x
    """
    try:
        r = SESSION.get(
            f"{BINANCE_BASE}/fapi/v1/klines",
            params={"symbol": symbol, "interval": interval, "limit": lookback + 2},
        ...
        # Önceki kapanmış mumların ortalama hacmi
        closed = data[:-1]
        closed_vols = [float(k[5]) for k in closed[-lookback:]]
        if not closed_vols:
            return None
        ...
        # Normalize: şu anki hız ile giderse bu mum ne kadar hacim üretir?
        projected_vol = cur_vol / elapsed_frac
        norm_ratio    = projected_vol / avg_vol

        # Gerçek anlık oran (sadece gelene kadar olan kısım / ortalama)
        raw_ratio = cur_vol / avg_vol

        pct_change = ((close_p - open_p) / open_p * 100) if open_p > 0 else 0
        direction  = "🟢 AL" if pct_change >= 0 else "🔴 SAT"

        remaining_ms  = close_time - server_time_ms
        remaining_min = max(0, round(remaining_ms / 60000))
        ...
        return {
            ...
            "raw_ratio":     round(raw_ratio, 2),       # o ana kadar olan kısım / ortalama
            "norm_ratio":    round(norm_ratio, 2),      # tahmini tam mum / ortalama (projeksiyon)
...
def scan(timeframe, min_norm_ratio, min_elapsed_pct, lookback,
    ...
    if err:
        ...
        if "451" in str(err):
            st.warning("451 hatası: Bu sunucu konumundan Binance Futures erişimi engellenmiş. "
                       "Frankfurt (Render) üzerinden çalıştırmanız gerekiyor.")
        return []
    ...
    for idx, coin in enumerate(symbols):
        ...
        if status_text:
            status_text.text(f"⏳ [{idx + 1}/{total}] {symbol} taranıyor...")
    ...
    if status_text:
        status_text.text(f"✅ Tamamlandı — {len(results)} coinde ani hacim artışı tespit edildi!")
...
st.caption("Kapanmamış mumda gelen hacmi, geçen süreye göre normalize ederek 'bu mumda normaldekinin kaç katı hacim geliyor?' sorusunu yanıtlar. Fiyat yönünden bağımsız.")

st.info(
    "**Normalize Hacim Oranı nasıl hesaplanır?**\n\n"
    "4 saatlik mumun %25'i geçmişken 100 birim hacim geldiyse → tahmini tam mum hacmi 400 birim. "
    "Son 20 mumun ortalaması 80 birim ise → **Normalize Oran = 400/80 = 5.0x**\n\n"
    "Bu, 'mum daha dolmadan normalin 5 katı hacim patlaması var' anlamına gelir."
)
...
with st.sidebar:
    ...
    timeframe = st.selectbox(
        ...
        index=5  # 1h varsayılan
    )

    min_norm_ratio = st.number_input(
        "Min Normalize Hacim Oranı (x)",
        min_value=1.0, max_value=50.0, value=3.0, step=0.5,
        help="Projeksiyon bazında normalin kaç katı hacim olmalı. 3x = normalin 3 katı."
    )

    min_elapsed_pct = st.number_input(
        "Min Mum İlerleme % (erken sinyalleri filtreler)",
        min_value=0, max_value=90, value=10, step=5,
        help="Mumun en az yüzde kaçı geçmiş olmalı. Çok düşük tutarsanız yanlış alarm artar."
    )

    lookback = st.number_input(
        "Ortalama Hesap İçin Bakılan Önceki Mum Sayısı",
    ...
    min_volume_24h = st.number_input(
        ...
        min_value=0, value=1_000_000, step=500_000
    )
...
c1.metric("Toplam Ani Hacim", len(results))
c2.metric("🟢 Yükselen", buy_c)
c3.metric("🔴 Düşen", sell_c)
c4.metric("En Yüksek Oran", f"{top_ratio}x")

st.subheader(f"📋 Ani Hacim Artışı Listesi (TF: {tf_label}, Min: {min_norm_ratio}x)")

if not results:
    st.info("Henüz tarama yapılmadı veya kritere uyan coin bulunamadı. '📊 HACİM TARAMASI BAŞLAT' butonuna basın.")
else:
    filter_dir = st.radio("Filtrele:", ["Tümü", "Sadece Yükselenler 🟢", "Sadece Düşenler 🔴"], horizontal=True)

    filtered = results
    if filter_dir == "Sadece Yükselenler 🟢":
        filtered = [r for r in results if "AL" in r["direction"]]
    elif filter_dir == "Sadece Düşenler 🔴":
    ...
    st.caption(
        "⚠️ 'Norm. Hacim Oranı' = Mevcut hızla giderse bu mumun biteceği tahmini toplam hacim / "
        "önceki N mumun ortalaması. 'Anlık Oran' = O ana kadar gelen gerçek hacim / ortalama. "
        "Mum henüz kapanmadığı için her iki değer de değişmeye devam eder."
    )
